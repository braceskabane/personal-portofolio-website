...
import { Message, ChatContextData } from '@/components/common/ChatBot/ChatBot.types';
import { EnhancedGeminiService, type GeminiConfig } from '@/services/ai/geminiService';
...
export const useMultilingualChatBot = ({ 
  ...
  const generateMultilingualMockResponse = useCallback(async (userMessage: string, language: SupportedLanguage): Promise<string> => {
    ...
    if (language === 'id') {
      if (lowerMessage.includes('pengalaman') || lowerMessage.includes('kerja') || lowerMessage.includes('karir')) {
      ...
      if (lowerMessage.includes('keahlian') || lowerMessage.includes('skill') || lowerMessage.includes('teknologi')) {
      ...
      if (lowerMessage.includes('proyek') || lowerMessage.includes('portfolio')) {
    ...
    // Spanish responses
    if (language === 'es') {
      if (lowerMessage.includes('experiencia') || lowerMessage.includes('trabajo') || lowerMessage.includes('carrera')) {
      ...
      if (lowerMessage.includes('habilidades') || lowerMessage.includes('tecnología') || lowerMessage.includes('tech')) {
  ...
  const sendMessage = useCallback(async (content: string, messageLanguage?: SupportedLanguage) => {
    if (!content.trim() || isLoading) return;

    let detectedLanguage = messageLanguage || currentLanguage;
    
    // Auto-detect language if enabled and Gemini is available
    if (enableAutoLanguageDetection && geminiServiceRef.current && !messageLanguage) {
      try {
        detectedLanguage = geminiServiceRef.current.detectLanguage(content);
        if (detectedLanguage !== currentLanguage) {
          console.log(`🔍 Language detected: ${detectedLanguage}, switching from ${currentLanguage}`);
          setCurrentLanguage(detectedLanguage);
        }
      } catch (error) {
        console.warn('⚠️ Language detection failed, using current language:', error);
      }
    }
    ...
    try {
      ...
      if (geminiServiceRef.current && apiKey) {
        ...
        response = await generateMultilingualMockResponse(content, detectedLanguage);
      }
      ...
      if (geminiServiceRef.current) {
        errorMessage = geminiServiceRef.current.getErrorMessage(detectedLanguage);
      } else {
  ...
  }, [messages, isLoading, context, currentLanguage, enableAutoLanguageDetection, generateMultilingualMockResponse, apiKey]);

  const clearChat = useCallback((language?: SupportedLanguage) => {
