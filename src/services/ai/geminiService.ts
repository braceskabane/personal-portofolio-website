...
export class EnhancedGeminiService {
  ...
  async generateResponse(
    ...
    try {
      ...
      const documentContext = this.enableDocumentContext
        ? this.documentService.getRelevantContext(
            message,
            language,
      ...
      const prompt = this.formatPromptWithDocuments(
        systemPrompt,
        message,
        language,
        conversationHistory,
      ...
      if (this.debug) {
        ...
        console.log("📤 Document Context:", documentContext || "(none)");
  ...
  detectLanguage(text: string): SupportedLanguage {
    ...
    const germanMatches = germanKeywords.filter((keyword) =>
      lowerText.includes(keyword),
    ).length;
  ...
  private buildEnhancedSystemPrompt(
    ...
    return `${languageInstruction}You are a professional AI assistant representing ${context.name}, a ${context.title}. You serve as a knowledgeable and articulate representative of this developer's portfolio.

YOUR ROLE:
- You are a professional portfolio assistant, not a general chatbot
- Answer questions about ${context.name}'s work, skills, experience, and projects with authority and depth
- Use the provided document context to give specific, detailed answers with concrete examples
- Maintain a professional yet approachable tone at all times
- Never make up information - only use what's provided in the context

DETAILED PERSONAL DOCUMENTS (use this for specific answers):
${documentContext}
...
RESPONSE GUIDELINES:
1. Always use the document context to provide specific, factual answers with concrete examples
2. Be professional, articulate, and showcase ${context.name}'s expertise naturally
3. When discussing projects or skills, reference specific technologies, outcomes, and achievements
4. If asked about something not in the documents, acknowledge the limitation gracefully and redirect to what you can help with
5. Keep responses concise but informative - aim for 2-4 well-crafted sentences
6. End responses with a natural follow-up question or call-to-action when appropriate
7. Never use overly casual language - maintain professional standards
${language === "en" ? "" : `8. CRITICAL: Always respond in ${languageName}.`}

REMEMBER: You are representing a professional developer's portfolio. Every response should reflect the quality and professionalism of ${context.name}'s work.`;
  }

  private formatPromptWithDocuments(
    systemPrompt: string,
    userMessage: string,
    language: SupportedLanguage,
    conversationHistory: Array<{ role: string; content: string }>,
