...
const Navigation: React.FC<{ activeSection: string }> = ({ activeSection }) => {
  ...
  return (
    <>
      <nav className="fixed top-0 w-full z-50 bg-[#0B1215] backdrop-blur-xl ">
      ...
      {/* Mobile Menu Sidebar */}
      <div
        className={`fixed top-0 right-0 w-80 max-w-[85vw] h-full z-50 bg-gradient-to-br from-gray-900/98 via-black/95 to-gray-900/98 backdrop-blur-xl border-l border-gray-700/50 shadow-2xl transform transition-all duration-300 ease-out md:hidden mobile-menu ${
          isMobileMenuOpen
...
const CustomCursor: React.FC<{ isChatOpen: boolean }> = ({ isChatOpen }) => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const overNav = target.closest("nav") || target.closest(".mobile-menu");
      const overChat = target.closest("[data-chatbot]");
      setIsOverNav(!!overNav || (isChatOpen && !overChat));
    };
  ...
  return (
    <div
      ref={cursorRef}
      className={`fixed pointer-events-none z-[70] mix-blend-difference hidden md:block transition-opacity duration-200 ${isOverNav ? "opacity-0" : ""}`}
...
export default function Portfolio() {
  ...
  return (
    <div className="min-h-screen bg-[#0B1215] text-black overflow-x-hidden">
      {/* Particle Background */}
      <ParticleBackground />
          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="text-center md:text-left mb-4 md:mb-0">
              <p className="text-gray-400">
                © 2025 {personalInfo?.name || "Muhammad Daffa' Fisabilillah"}.
                All rights reserved.
              </p>
      ...
      {process.env.NODE_ENV === "development" && (
        <div className="fixed bottom-4 left-4 z-50 bg-black/80 backdrop-blur-xl border border-gray-700 rounded-lg p-4 text-xs max-w-xs">
          ...
          <div className="space-y-1 text-gray-400">
            <div>
              Active: <span className="text-cyan-400">{activeSection}</span>
            </div>
            <div>Personal Info: {personalInfo ? "✅" : "❌"}</div>
            <div>
              Projects: <span className="text-cyan-400">{projects.length}</span>
            </div>
            <div>
              Experience:{" "}
              <span className="text-cyan-400">{experience.length}</span>
            </div>
            <div>
              Skills: <span className="text-cyan-400">{skills.length}</span>
            </div>
            <div>
              Loading: {Object.values(loading).some((l) => l) ? "⏳" : "✅"}
            </div>
            <div>
              Errors: {Object.values(error).some((e) => e) ? "❌" : "✅"}
            </div>
          </div>
        </div>
      )}
